...
// ==========================================
// SKRIP DETEKSI ERROR (DEBUGGING)
// ==========================================
if (!isset($_GET['NO']) || empty($_GET['NO'])) {
    echo "<div style='padding: 20px; background: #fef2f2; color: #ef4444; border: 1px solid #fee2e2; margin: 20px; border-radius: 8px;'>";
    echo "<h3>❌ Error: Parameter 'NO' Tidak Ditemukan di URL!</h3>";
    echo "Penyebab: Tombol edit di file <b>anggota/tampil.php</b> kemungkinan salah menulis link. Pastikan tulisannya <code>edit.php?NO=...</code> (huruf NO harus kapital).<br>";
    echo "<a href='tampil.php' style='display:inline-block; margin-top:10px; color:#4f46e5;'>← Kembali ke Daftar Anggota</a>";
    echo "</div>";
    exit;
}

$NO = mysqli_real_escape_string($conn, $_GET['NO']);
$query = mysqli_query($conn, "SELECT * FROM anggota WHERE NO='$NO'");

if (!$query) {
    echo "<div style='padding: 20px; background: #fef2f2; color: #ef4444; border: 1px solid #fee2e2; margin: 20px; border-radius: 8px;'>";
    echo "<h3>❌ Error: Query ke Database Gagal!</h3>";
...
// Jika ID ada di URL tapi data tidak ditemukan di tabel anggota
if(!$d){
    echo "<div style='padding: 20px; background: #fef2f2; color: #ef4444; border: 1px solid #fee2e2; margin: 20px; border-radius: 8px;'>";
    echo "<h3>❌ Error: Data Anggota Tidak Ditemukan!</h3>";
    echo "Penyebab: Nilai NO yang dikirim adalah <b>" . htmlspecialchars($NO) . "</b>, tetapi nilai tersebut tidak ada di dalam kolom <b>NO</b> tabel anggota database Anda.<br>";
    echo "<a href='tampil.php' style='display:inline-block; margin-top:10px; color:#4f46e5;'>← Kembali ke Daftar Anggota</a>";
    echo "</div>";
    exit;
}
...
<head>
    ...
    <title>Edit Anggota - NusaBaca</title>
...
<div class="main">
    ...
    <h3 class="fw-bold text-dark mb-4">Edit Informasi Anggota</h3>

    <div class="form-card">
        <form method="POST">
            <div class="mb-4">
                <label class="form-label">Nama Lengkap</label>
                <input type="text" name="nama" class="form-control" value="<?= isset($d['nama']) ? htmlspecialchars($d['nama']) : ''; ?>" required>
            </div>

            <div class="mb-4">
                <label class="form-label">Alamat Rumah</label>
                <textarea name="alamat" class="form-control" rows="3" required><?= isset($d['alamat']) ? htmlspecialchars($d['alamat']) : ''; ?></textarea>
            </div>
